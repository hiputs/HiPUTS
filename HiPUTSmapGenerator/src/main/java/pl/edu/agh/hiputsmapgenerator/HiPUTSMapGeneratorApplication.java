...
public class HiPUTSMapGeneratorApplication {
  ...
  private static final Pair mapDims = new Pair(32, 32); // all nodes in map for one worker (ONE WORKER MAP)
  ...
  private static void saveEdges(List<Lane> edges, int workerCount) throws IOException {
    PrintWriter writer = new PrintWriter(getDirPath(workerCount) + "/lanes.csv", StandardCharsets.UTF_8);
