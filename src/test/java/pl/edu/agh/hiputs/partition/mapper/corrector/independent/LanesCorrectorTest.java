...
public class LanesCorrectorTest {
  ...
  private final LanesCorrector corrector =
      new LanesCorrector(new StandardLanesCreator(new StandardOsmAndRoundaboutOnewayProcessor()),
          List.of(new OnBendSuccessorAllocator(new StandardOsmTurnProcessor()),
              new OnCrossroadSuccessorAllocator(new DefaultPairingIncomingWithOutgoings(),
                  new StandardOsmTurnProcessor(), new FixedAngleRangeTurnMapper(), new ByAngleEdgeSorter())));
  ...
  public void happyPathCreatingLanesAndAssigningSuccessors() {
    ...
    Edge<JunctionData, WayData> inEdge =
        new Edge<>("10", WayData.builder().tags(Collections.singletonMap("lanes", "4")).build());
    Edge<JunctionData, WayData> outEdge1 = new Edge<>("02", WayData.builder().tags(Collections.emptyMap()).build());
    Edge<JunctionData, WayData> outEdge2 = new Edge<>("03", WayData.builder().tags(Collections.emptyMap()).build());
    ...
    Graph<JunctionData, WayData> graph = new GraphBuilder<JunctionData, WayData>().addNode(startNode)
        .addNode(crossroad)
        .addNode(nextNode1)
        .addNode(nextNode2)
        .addEdge(inEdge)
    ...
    // then
    Assertions.assertEquals(2, returned.getEdges()
        .values()
        .stream()
