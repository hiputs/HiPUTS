...
public class StandardComplexCrossroadsUpdaterTest {
  ...
  private final StandardCrossroadDeterminer determiner =
      Mockito.mock(StandardCrossroadDeterminer.class);
  private final ComplexCrossroadsRepository repository = new ComplexCrossroadsRepositoryImpl();
  private final StandardComplexCrossroadsUpdater updater = new StandardComplexCrossroadsUpdater(
      determiner, repository, modelConfigService);
  ...
  public void oneCCRepoAndOneFarCrossroadNode() {
    ...
    updater.extendWithNodes(Set.of(nodeA, nodeC));
    Assertions.assertEquals(1, repository.getComplexCrossroads().size());
    Assertions.assertTrue(repository.getComplexCrossroads().stream()
        .noneMatch(cc -> cc.getNodesIdsIn().contains(nodeC.getId())));
  }
  ...
  public void oneCCRepoAndOneCloseCrossroadNode() {
    ...
    Assertions.assertEquals(1, repository.getComplexCrossroads().size());
    Assertions.assertTrue(repository.getComplexCrossroads().stream()
        .allMatch(cc -> cc.getNodesIdsIn().contains(nodeC.getId())));
  }
  ...
  public void twoCCsRepoAndOneConnectedCrossroadNodeCloseToBothCC() {
    ...
    Assertions.assertEquals(1, repository.getComplexCrossroads().size());
    Assertions.assertTrue(repository.getComplexCrossroads().stream()
        .allMatch(cc -> cc.getNodesIdsIn().contains(nodeE.getId())));
  }
