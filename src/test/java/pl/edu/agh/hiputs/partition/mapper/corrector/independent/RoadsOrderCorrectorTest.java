...
public class RoadsOrderCorrectorTest {
  ...
  public void sortEmptyGraph() {
    ...
    List<Edge<JunctionData, WayData>> incomingOrder =
        returned.getNodes().values().stream().findFirst().map(Node::getIncomingEdges).orElse(Collections.emptyList());
    List<Edge<JunctionData, WayData>> outgoingOrder =
        returned.getNodes().values().stream().findFirst().map(Node::getOutgoingEdges).orElse(Collections.emptyList());
  ...
  public void sortSingleEdges() {
    ...
    Graph<JunctionData, WayData> graph =
        new GraphBuilder<JunctionData, WayData>().addNode(crossroad).addEdge(inEdge1).addEdge(outEdge1).build();
    Graph<JunctionData, WayData> returned = corrector.correct(graph);
    List<Edge<JunctionData, WayData>> incomingOrder =
        returned.getNodes().values().stream().findFirst().map(Node::getIncomingEdges).orElse(Collections.emptyList());
    List<Edge<JunctionData, WayData>> outgoingOrder =
