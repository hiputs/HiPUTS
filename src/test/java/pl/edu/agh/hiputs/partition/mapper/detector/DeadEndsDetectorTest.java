...
public class DeadEndsDetectorTest {
  ...
  private final DeadEndsDetector detector =
      new DeadEndsDetector(deadEndsCorrectorStrategyFactory, standardDetectorStrategyExecutor, finders);
  @Captor
  ...
  public void happyPath() {
    ...
    Graph<JunctionData, WayData> graph = new Graph.GraphBuilder<JunctionData, WayData>().addNode(nodeA)
        ...
        .addEdge(edge5)
        .build();
    Mockito.when(deadEndsCorrectorStrategyFactory.getFromConfiguration())
        .thenReturn(new AddReversesOnDeadEndsFixer(new StandardEdgeReflector()));
