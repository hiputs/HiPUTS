...
public class AddNewEdgeTypesFixerTest {
  ...
  public void oneEdgeWithIncompatibility() {
    ...
    Node<JunctionData, WayData> nodeA =
        new Node<>("A", JunctionData.builder().isCrossroad(false).lat(1.0).lon(1.0).build());
    Node<JunctionData, WayData> nodeB =
        new Node<>("B", JunctionData.builder().isCrossroad(false).lat(2.0).lon(2.0).build());
    TypeIncompatibility typeIncompatibility = new TypeIncompatibility("motorway", "motorway_link", edge1);
  ...
  public void threeEdgesAndTwoSameIncompatibilities() {
    ...
    Node<JunctionData, WayData> nodeA =
        new Node<>("A", JunctionData.builder().isCrossroad(false).lat(1.0).lon(1.0).build());
    Node<JunctionData, WayData> nodeB =
        new Node<>("B", JunctionData.builder().isCrossroad(false).lat(1.0).lon(4.0).build());
    Node<JunctionData, WayData> nodeC =
        new Node<>("C", JunctionData.builder().isCrossroad(false).lat(1.0).lon(7.0).build());
    Node<JunctionData, WayData> nodeD =
        new Node<>("D", JunctionData.builder().isCrossroad(false).lat(1.0).lon(10.0).build());
    ...
    // then
    Graph<JunctionData, WayData> resultGraph =
        fixer.fixFoundIncompatibilities(List.of(typeIncompatibility1, typeIncompatibility2), graph);
    ...
    Assertions.assertEquals(2, resultGraph.getEdges()
        .values()
        .stream()
        .filter(edge -> edge.getData().getTags().containsKey("highway") && edge.getData()
            .getTags()
            .get("highway")
  ...
  public void threeEdgesAndThreeDifferentIncompatibilities() {
    ...
    // then
    Graph<JunctionData, WayData> resultGraph =
        fixer.fixFoundIncompatibilities(List.of(typeIncompatibility1, typeIncompatibility2, typeIncompatibility3),
            graph);
