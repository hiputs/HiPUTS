  ...
  public void run(int step) {
    try {
      ...
      List<Runnable> decisionStageTasks = mapFragment.getLocalRoadIds()
          .stream()
          .map(roadId -> new RoadDecisionStageTask(mapFragment, roadId, carGeneratorService, // todo sprawdzic ten task
      ...
      List<Runnable> updateStageTasks = mapFragment.getLocalRoadIds().stream()
          .map(roadId -> new RoadUpdateStageTask(mapFragment, roadId)) //todo sprawdzic ten plik
          .collect(Collectors.toList());
