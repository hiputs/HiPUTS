...
public class FileGeneratorServiceImpl implements FileGeneratorService {
  ...
  public void generateFiles(MapFragment fragment) {
    ...
    var endStep = timeBasedCarGeneratorConfig.getConfigEntries()
        .get(timeBasedCarGeneratorConfig.getConfigEntries().size() - 1)
        .getEndStep();
  ...
  private void generateFileForPatch(Patch patch, MapFragment mapFragment, int startStep, int endStep) {
    ...
    try (var fw = new FileWriter(filePath, false); var bw = new BufferedWriter(fw); var out = new PrintWriter(bw)) {
  ...
  private void writeRoutesToFile(Patch patch, int startStep, int endstep, MapFragment mapFragment, PrintWriter out) {
    List<Pair<RouteWithLocation, Integer>> routesWithStep =
        fileGenerator.generateRouteFileInput(patch, startStep, endstep, mapFragment, mapRepository,
