...
public class ShortestPathRouteGenerator implements RouteGenerator {
  ...
  @Override
  public List<RouteWithLocation> generateRoutesFromMapFragment(Patch startPatch, int numberOfRoutes,
  ...
  @Override
  public List<RouteWithLocation> generateRoutesFromMapRepository(Patch startPatch, int numberOfRoutes,
      MapRepository mapRepository) {
    //    TODO: consider adding property for which type of path finder you want to use, when different path finder
  ...
  private List<RouteWithLocation> generateRoutes(Patch startPatch, int numberOfRoutes, List<Patch> patches,
      PathFinder pathFinder) {
    List<RouteWithLocation> routes = new ArrayList<>();
    for (int i = 0; i < numberOfRoutes; i++) {
      try {
        RoadId startLaneId = getRandomWeightedRoadId(startPatch);

        Patch endPatch = getEndingPatch(patches);
        RoadId endLineId = getRandomWeightedRoadId(endPatch);
  ...
  private RoadId getRandomWeightedRoadId(Patch patch) {
    double totalLength = patch.getRoadsLength();
    double startingPoint = ThreadLocalRandom.current().nextDouble(totalLength);

    for (RoadId roadId : patch.getRoadIds()) {
      startingPoint -= patch.getRoadReadable(roadId).getLength();
      if (startingPoint <= 0) {
        return roadId;
      }
    }
