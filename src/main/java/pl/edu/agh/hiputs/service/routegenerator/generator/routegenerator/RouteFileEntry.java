...
public class RouteFileEntry {
  ...
  public String toFileLine() {
    return concatenate(String.valueOf(step), String.valueOf(carLength), String.valueOf(maxSpeed), String.valueOf(speed),
        routeToString());
  }

  private String routeToString() {
    return route.getRouteElements()
        .stream()
        .map(element -> element.getJunctionId().getValue() + COMMA + element.getOutgoingRoadId().getValue() + COMMA)
