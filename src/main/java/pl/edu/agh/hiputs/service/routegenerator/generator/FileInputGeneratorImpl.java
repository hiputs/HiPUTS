...
public class FileInputGeneratorImpl implements FileInputGenerator {
  ...
  @Override
  public List<Pair<RouteWithLocation, Integer>> generateRouteFileInput(Patch patch, int startStep, int endStep,
      MapFragment mapFragment, MapRepository mapRepository, Boolean fromMapFragment) {
    var patchTotalLaneLength = patch.getRoadsLength();
    ...
    for (int step = startStep; step <= endStep; step++) {
      ...
      List<RouteWithLocation> newRoutes =
          fromMapFragment ? routeGenerator.generateRoutesFromMapFragment(patch, carsAmountToGenerate, mapFragment)
