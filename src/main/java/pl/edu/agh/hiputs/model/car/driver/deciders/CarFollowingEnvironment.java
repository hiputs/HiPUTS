...
public class CarFollowingEnvironment {
  ...
  public String toString(){
    return getClass().getSimpleName() + "(distance=" + distance
        + ", followingCar=Optional" + (followingCar.map(carReadable -> "[" + carReadable.getCarId() + "]")
        .orElse(".empty"))
        + ", prevCrossroadId=Optional" + (prevCrossroadId.map(junctionId -> "[" + junctionId + "]").orElse(".empty"))
