...
public class MobilModel implements ILaneChangeChecker {
   ...
   *
   * new_a_car - a_car + politeness_factor * (new_a_new_follower - a_new_follower + new_a_old_follower - a_old_follower)
  ...
  public MobilModelDecision makeDecision(CarReadable car, LaneId targetLaneId, double politenessFactor,
    ...
    double overallAcceleration, carAcceleration, carAccelerationLC, oldFollowerAcceleration = 0.0,
        oldFollowerAccelerationLC = 0.0, newFollowerAcceleration = 0.0, newFollowerAccelerationLC = 0.0;
    ...
    if (newFollowingCarEnv.getFollowingCar().isPresent()) {
      ...
      newFollowerAcceleration = idm.makeDecision(newFollower,
          new CarPrecedingEnvironment(newPrecedingCarEnv.getPrecedingCar(), Optional.empty(),
              newPrecedingCarEnv.getDistance() + newFollowingCarEnv.getDistance()), roadStructureReader);
    }
    ...
    if (newFollowingCarEnv.getFollowingCar().isPresent()) {
      ...
      newFollowerAccelerationLC = idm.makeDecision(newFollower,
          new CarPrecedingEnvironment(newPrecedingCarEnv.getPrecedingCar(), Optional.empty(),
              newFollowingCarEnv.getDistance()), roadStructureReader);
    }
    if (followingCarEnv.getFollowingCar().isPresent()) {
      oldFollowerAccelerationLC = idm.makeDecision(followingCarEnv.getFollowingCar().get(),
          new CarPrecedingEnvironment(carPrecedingEnvironment.getPrecedingCar(), Optional.empty(),
              followingCarEnv.getDistance() + carPrecedingEnvironment.getDistance()), roadStructureReader);
    }
    ...
    if ((!skipOverallAccelerationCheck && (overallAcceleration > ACCELERATION_THRESHOLD) && (newFollowerAccelerationLC
        > (-1) * BE_SAFE)) || (skipOverallAccelerationCheck && (newFollowerAccelerationLC > (-1) * BE_SAFE))) {
