...
public class TicketServiceImpl implements TicketService, Subscriber {
  ...
  @PostConstruct
  void init() {
    if (configuration.isTicketActive()) {
    ...
    public void run() {
      ...
      while (true) {
        ...
        if(!newMapFragment.isEmpty()){
          ...
          if (neighbour.hashCode() > configuration.getMapFragmentId()
    ...
    private void selectTicket() {
      ...
      if (message != null) {
        ...
        try {
          ...
          messageSenderService.send(neighbour, new SelectTicketMessage(configuration.getMapFragmentId().getId(),
                  selectTicket));
        } catch (IOException e) {
