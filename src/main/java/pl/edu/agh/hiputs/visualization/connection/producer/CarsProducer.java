...
public class CarsProducer {
  ...
  private boolean checkIsLaneIntersectingRegion(LaneReadable laneReadable, MapFragment mapFragment,
      ROIRegion roiRegion) {
    return checkIsJunctionInsideRegion(mapFragment.getRoadReadable(laneReadable.getRoadId()).getIncomingJunctionId(),
        mapFragment, roiRegion) || checkIsJunctionInsideRegion(
        mapFragment.getRoadReadable(laneReadable.getRoadId()).getOutgoingJunctionId(), mapFragment, roiRegion);
  }

  private boolean checkIsJunctionInsideRegion(JunctionId junctionId, MapFragment mapFragment, ROIRegion roiRegion) {
    return Optional.ofNullable(mapFragment.getJunctionReadable(junctionId))
        .stream()
  ...
  private static CarMessage createCarMessage(CarReadable car, MapFragment mapFragment) {
    ...
    return CarMessage.newBuilder()
        ...
        .setNode1Id(mapFragment.getRoadReadable(carLane.getRoadId()).getIncomingJunctionId().getValue())
        .setNode2Id(mapFragment.getRoadReadable(carLane.getRoadId()).getOutgoingJunctionId().getValue())
  ...
  private void sendCarMessageList(List<CarMessage> carMessageList, MapFragment mapFragment, int iterationNumber) {
    CarsMessage carsMessage =
        CarsMessage.newBuilder().setIterationNumber(iterationNumber).addAllCarsMessages(carMessageList).build();
