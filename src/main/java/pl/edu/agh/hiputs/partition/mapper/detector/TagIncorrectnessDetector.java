...
public class TagIncorrectnessDetector implements Detector{
  ...
  private String formatReportForEdges(List<Pair<String, List<Edge<JunctionData, WayData>>>> edgeFindersResult) {
    ...
    edgeFindersResult.forEach(edgeFinderResult -> edgesReportBuilder.append(String.format("\"%s\": [\n%s]\n",
        edgeFinderResult.getLeft(),
        edgeFinderResult.getRight().stream()
            .map(Edge::getId)
  ...
  private String formatReportForNodes(List<Pair<String, List<Node<JunctionData, WayData>>>> nodeFindersResult) {
    ...
    nodeFindersResult.forEach(nodeFinderResult -> nodesReportBuilder.append(String.format("\"%s\": [\n%s]\n",
        nodeFinderResult.getLeft(),
        nodeFinderResult.getRight().stream()
            .map(Node::getId)
            .reduce("", (current, next) -> current + String.format("\t%s,\n", next))
    )));
