...
public class AddNewEdgeTypesFixer implements TypesFixer {
  ...
  private void divideEdgeIntoTwoWithNewNode(TypeIncompatibility typeIncompatibility,
      Graph<JunctionData, WayData> graph) {
    Edge<JunctionData, WayData> foundEdge = typeIncompatibility.getImpactedEdge();

    boolean isPredecessorMotorway = foundEdge.getSource()
        .getIncomingEdges()
        .stream()
        .anyMatch(edge -> isRequiredType(edge, typeIncompatibility.getImpactedType()));
    boolean isSuccessorMotorway = foundEdge.getTarget()
        .getOutgoingEdges()
        .stream()
        .anyMatch(edge -> isRequiredType(edge, typeIncompatibility.getImpactedType()));
  ...
  private boolean isRequiredType(Edge<JunctionData, WayData> edge, String type) {
    return edge.getData().getTags().containsKey(highwayKey) && edge.getData().getTags().get(highwayKey).equals(type);
  }

  private Edge<JunctionData, WayData> splitEdgeAndCreateNode(Graph<JunctionData, WayData> graph,
      Edge<JunctionData, WayData> currentEdge, int splitSizeByPart, boolean fromLeft, String requiredType) {
