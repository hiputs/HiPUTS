...
public class ComplexCrossroadsCorrector implements Corrector {
  ...
  public Graph<JunctionData, WayData> correct(Graph<JunctionData, WayData> graph) {
    ...
    List<CrossroadTransformation> preparedTransformations = complexCrossroadsRepository.getComplexCrossroads()
        .stream()
        .filter(
            crossroad -> crossroad.getNodesIdsIn().stream().allMatch(nodeId -> graph.getNodes().containsKey(nodeId)))
    ...
    Map<Node<JunctionData, WayData>, Node<JunctionData, WayData>> oldNode2NewNode = preparedTransformations.stream()
        .flatMap(crossroadTransformation -> crossroadTransformation.getOldNodes()
            .stream()
    ...
    preparedTransformations.forEach(transformation -> graphBuilder.addNode(transformation.getNewNode()));
    ...
    preparedTransformations.forEach(transformation -> {
      transformation.getExternalIncomingEdges()
          .stream()
      ...
      transformation.getExternalOutgoingEdges()
          .stream()
          .map(edge -> recreateEdge(edge,     // recreation for new id and length
              oldNode2NewNode.getOrDefault(transformation.getNewNode(), transformation.getNewNode()),
  ...
  private CrossroadTransformation transform(ComplexCrossroad complexCrossroad, Graph<JunctionData, WayData> graph) {
    ...
    transformation.setOldNodes(complexCrossroad.getNodesIdsIn()
        .stream()
        .map(nodeId -> graph.getNodes().get(nodeId))
        .collect(Collectors.toSet()));

    // needed for filtering out during new graph building
    transformation.setOldEdges(transformation.getOldNodes()
        .stream()
    ...
    transformation.setNewNode(new Node<>(UUID.randomUUID().toString(),
        mergeAllJunctionData(transformation.getOldNodes().stream().map(Node::getData).toList())));

    // external incoming edges, they must point new node at target
    transformation.setExternalIncomingEdges(transformation.getOldNodes()
        .stream()
    ...
    transformation.setExternalOutgoingEdges(transformation.getOldNodes()
        .stream()
  ...
  private Edge<JunctionData, WayData> recreateEdge(Edge<JunctionData, WayData> edge, Node<JunctionData, WayData> source,
  ...
  private JunctionData mergeAllJunctionData(List<JunctionData> allJunctionData) {
    Map<String, Map<String, Long>> keyValueStatistics = allJunctionData.stream()
        ...
        .collect(Collectors.groupingBy(Entry::getKey, Collectors.groupingBy(Entry::getValue, Collectors.counting())));

    Map<String, String> getMostUsedValuesForEachKey = keyValueStatistics.entrySet()
        .stream()
        .map(entry -> Map.entry(entry.getKey(),
            entry.getValue().entrySet().stream().max(Comparator.comparingLong(Entry::getValue)).map(Entry::getKey)))
        .filter(entry -> entry.getValue().isPresent())
        .map(entry -> Map.entry(entry.getKey(), entry.getValue().get()))
        .collect(Collectors.toMap(Entry::getKey, Entry::getValue));

    double latitude =
        allJunctionData.stream().map(JunctionData::getLat).mapToDouble(lat -> lat).average().orElseThrow();

    double longitude =
        allJunctionData.stream().map(JunctionData::getLon).mapToDouble(lon -> lon).average().orElseThrow();
