...
public class StandardLanesCreator implements LanesCreator {
  ...
  private List<LaneData> createLanesForEdge(WayData wayData) {
    if (wayData.getTags().containsKey(LANES_TOTAL_KEY)) {
      if (wayData.getTags().containsKey(LANES_FORWARD_KEY) && wayData.getTags().containsKey(LANES_BACKWARD_KEY)
        ...
        return wayData.isTagsInOppositeMeaning() ? createNoLanes(
            retrieveLanesNo(wayData, LANES_BACKWARD_KEY) + retrieveLanesNo(wayData, LANES_BOTH_WAYS_KEY))
            : createNoLanes(
                retrieveLanesNo(wayData, LANES_FORWARD_KEY) + retrieveLanesNo(wayData, LANES_BOTH_WAYS_KEY));
      }
        ...
        return wayData.isTagsInOppositeMeaning() ? createNoLanes(retrieveLanesNo(wayData, LANES_BACKWARD_KEY))
        ...
        return wayData.isTagsInOppositeMeaning() ? createNoLanes(
            retrieveLanesNo(wayData, LANES_TOTAL_KEY) - retrieveLanesNo(wayData, LANES_FORWARD_KEY))
            : createNoLanes(retrieveLanesNo(wayData, LANES_FORWARD_KEY));
      }
