...
public class DirectBridgesConnectFixer implements ConnectFixer {
  ...
  private List<Edge<JunctionData, WayData>> createDirectedBridgesOnGraph(List<StronglyConnectedComponent> sCCs,
    ...
    Map<String, StronglyConnectedComponent> nodesIds2SCC = sCCs.stream()
        .flatMap(scc -> scc.getNodesIds().stream().map(nodeId -> Pair.of(nodeId, scc)))
    ...
    Map<String, StronglyConnectedComponent> externalEdgesIds2PointingSCC = sCCs.stream()
        .flatMap(scc -> scc.getExternalEdgesIds()
            .stream()
  ...
  private Stream<String> uniqueMinExternalEdges(Set<String> edgesIds,
      Map<String, StronglyConnectedComponent> externalEdgesIds2PointingSCC, Graph<JunctionData, WayData> graph) {
