...
public class RelationsRequirement implements Requirement {
  ...
  private boolean areRestrictionsConsidered(Node<JunctionData, WayData> node) {
    ...
    Set<String> outgoingEdgesOnRestrictions =
        node.getData().getRestrictions().stream().map(Restriction::getToEdgeId).collect(Collectors.toSet());

    boolean emptyOutgoingByNoRestrictions = node.getOutgoingEdges()
        .stream()
    ...
    boolean moreOnlyRestrictions = node.getData()
        .getRestrictions()
        .stream()
    ...
    Map<String, Edge<JunctionData, WayData>> edges2ConnectingNode =
        Stream.concat(node.getIncomingEdges().stream(), node.getOutgoingEdges().stream())
            .collect(Collectors.toMap(Edge::getId, Function.identity()));

    return node.getData().getRestrictions().stream().allMatch(restriction -> {
      ...
      if (restriction.getType().toString().startsWith("ONLY_")) {
        ...
        return new HashSet<>(toEdge.getData().getLanes()).containsAll(fromEdge.getData()
            .getLanes()
            .stream()
