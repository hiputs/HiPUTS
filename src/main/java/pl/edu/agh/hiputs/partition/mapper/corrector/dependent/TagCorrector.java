...
public class TagCorrector implements Corrector {
    ...
    edges.forEach(edge -> {
      ...
      if (tag.equals("maxspeed")) {
        if (predecessor.isPresent() && successor.isPresent()) {
          edge.getData()
              .getTags()
              .put(tag, avgSpeed(predecessor.get().getData().getTags().get(tag),
                  successor.get().getData().getTags().get(tag)));
        } else {
          predecessor.ifPresent(candidate -> edge.getData().getTags().put(tag, candidate.getData().getTags().get(tag)));

          successor.ifPresent(candidate -> edge.getData().getTags().put(tag, candidate.getData().getTags().get(tag)));
        }
      } else {
        predecessor.ifPresent(candidate -> edge.getData().getTags().put(tag, candidate.getData().getTags().get(tag)));
  ...
  private void fixNodes(String tag, List<Node<JunctionData, WayData>> nodes) {
    if (tag.equals("traffic_signals")) {
      nodes.forEach(node -> node.getData()
          .getTags()
          .entrySet()
          .stream()
