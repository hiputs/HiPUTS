...
public class RoadsOrderCorrector implements Corrector {
  ...
  public Graph<JunctionData, WayData> correct(Graph<JunctionData, WayData> graph) {
    ...
    graph.getNodes().values().forEach(node -> {
      Optional.of(node).filter(nodeToCheck -> nodeToCheck.getIncomingEdges().size() >= 2).ifPresent(checkedNode -> {
        List<Edge<JunctionData, WayData>> sortedIncomingEdges =
            edgeSorter.getSorted(checkedNode.getIncomingEdges(), checkedNode.getIncomingEdges().get(0),
                incomingEdge -> incomingEdge.getSource().getData(), incomingEdge -> incomingEdge.getTarget().getData());
      ...
      Optional.of(node).filter(nodeToCheck -> nodeToCheck.getOutgoingEdges().size() >= 2).ifPresent(checkedNode -> {
        List<Edge<JunctionData, WayData>> sortedOutgoingEdges =
            edgeSorter.getSorted(checkedNode.getOutgoingEdges(), checkedNode.getOutgoingEdges().get(0),
                outgoingEdge -> outgoingEdge.getTarget().getData(), outgoingEdge -> outgoingEdge.getSource().getData());
