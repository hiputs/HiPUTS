...
public class RestrictionsCorrector implements Corrector {
  ...
  public Graph<JunctionData, WayData> correct(Graph<JunctionData, WayData> graph) {
    ...
    restrictionAware.provideRestrictions(
        foundRestrictions.stream().filter(restriction -> isUpToDate(restriction, graph)).collect(Collectors.toSet()));

    return graph;
  }

  private boolean isUpToDate(Restriction restriction, Graph<JunctionData, WayData> graph) {
    return graph.getEdges().containsKey(restriction.getFromEdgeId()) && graph.getNodes()
        .containsKey(restriction.getViaNodeId()) && graph.getEdges().containsKey(restriction.getToEdgeId());
  }
  ...
  private Optional<String> findReverseEdge(String edgeId, Graph<JunctionData, WayData> graph) {
    ...
    return processingEdge.getTarget()
        .getOutgoingEdges()
        .stream()
