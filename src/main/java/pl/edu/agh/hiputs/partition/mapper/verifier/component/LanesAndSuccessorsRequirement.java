...
public class LanesAndSuccessorsRequirement implements Requirement{
  ...
  public boolean isSatisfying(Graph<JunctionData, WayData> graph) {
    return areLanesDefined(graph) &&
        areSuccessorsAssignedOnBends(graph) &&
        areSuccessorsAssignedOnCrossroads(graph) &&
        areSuccessorsNotOverlappingWithLanes(graph) &&
        areSuccessorsFromOutgoingEdgesOnly(graph);
  }
  ...
  private boolean areLanesDefined(Graph<JunctionData, WayData> graph) {
    return graph.getEdges().values().stream()
        .noneMatch(edge -> edge.getData().getLanes().isEmpty());
  }
  ...
  private boolean areSuccessorsFromOutgoingEdgesOnly(Graph<JunctionData, WayData> graph) {
    return graph.getEdges().values().stream()
        .allMatch(edge -> {
          Set<LaneData> outgoingLanes = edge.getTarget().getOutgoingEdges().stream()
          ...
          return edge.getData().getLanes().stream()
              .flatMap(lane -> lane.getAvailableSuccessors().stream())
