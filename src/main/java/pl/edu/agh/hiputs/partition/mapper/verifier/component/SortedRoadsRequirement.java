...
public class SortedRoadsRequirement implements Requirement {
  ...
  private boolean areRoadsSortedOnNode(Node<JunctionData, WayData> node) {
    return arePointsSorted(Point.convertFromCoords(node.getData()),
        node.getIncomingEdges().stream().map(edge -> Point.convertFromCoords(edge.getSource().getData())).toList())
        && arePointsSorted(Point.convertFromCoords(node.getData()),
        node.getOutgoingEdges().stream().map(edge -> Point.convertFromCoords(edge.getTarget().getData())).toList());
  }
  ...
  private boolean arePointsSorted(Point centerPoint, List<Point> points) {
    ...
    List<Vector> vectors = points.stream().map(point -> new Vector(centerPoint, point)).toList();

    Vector mainVector = vectors.get(0);
    List<Double> angles =
        vectors.stream().skip(1).map(vector -> Vector.calculateAngleBetween(mainVector, vector)).toList();
