...
import pl.edu.agh.hiputs.partition.mapper.corrector.independent.util.speed.component.table.loader.FileDataLoader;
import pl.edu.agh.hiputs.partition.mapper.corrector.independent.util.speed.component.table.validator.DataValidator;
...
public class SpeedLimitsRepositoryImpl implements SpeedLimitsRepository {
  ...
  private void init() {
    try {
      ...
      mapOfCountriesWithSpeedLimits =
          speedLimits.stream().collect(Collectors.toMap(SpeedLimitRecord::getCountryId, Function.identity()));
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
