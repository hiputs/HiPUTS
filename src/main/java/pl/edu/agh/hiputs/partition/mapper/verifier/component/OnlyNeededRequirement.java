...
public class OnlyNeededRequirement implements Requirement {
  ...
  public boolean isSatisfying(Graph<JunctionData, WayData> graph) {
    return graph.getNodes().values().stream().allMatch(this::isNodeComplete) && graph.getEdges()
        .values()
        .stream()
        .allMatch(this::isEdgeComplete);
  }
  ...
  private boolean isEdgeComplete(Edge<JunctionData, WayData> edge) {
    return Objects.nonNull(edge.getSource()) && Objects.nonNull(edge.getTarget()) && edge.getData()
        .getTags()
        .containsKey(highwayKey) && Arrays.stream(modelConfigService.getModelConfig().getWayTypes())
        .anyMatch(wayType -> edge.getData().getTags().get(highwayKey).equals(wayType));
  }
}
