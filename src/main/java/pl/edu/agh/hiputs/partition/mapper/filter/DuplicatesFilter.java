...
public class DuplicatesFilter implements Filter {
  ...
  private Collection<OsmNode> collectOrMergeNodesByLocation(Collection<OsmNode> nodes) {
    ...
    nodes.forEach(nextNode -> {
      ...
      if (nodesByLocationAggregator.containsKey(nodeLocationKey)) {
        ...
        nodesByLocationAggregator.put(nodeLocationKey,
            mergeNodes(nodesByLocationAggregator.get(nodeLocationKey), nextNode));
      } else {
  ...
  private Collection<OsmWay> collectOrDeleteWaysById(Collection<OsmWay> ways) {
    ...
    ways.forEach(nextWay -> {
      if (waysByIdAggregator.containsKey(nextWay.getId())) {
        ...
        if (Objects.nonNull(nextWay.getMetadata()) && Objects.nonNull(
            waysByIdAggregator.get(nextWay.getId()).getMetadata())
            && nextWay.getMetadata().getTimestamp() < waysByIdAggregator.get(nextWay.getId())
            .getMetadata()
            .getTimestamp()) {
  ...
  private OsmNode mergeNodes(OsmNode first, OsmNode second) {
    if (Objects.isNull(first.getMetadata()) || Objects.isNull(second.getMetadata())) {
      return new Random().nextBoolean() ? new Node(first.getId(), first.getLongitude(), first.getLatitude(),
          mergeTags(first, second))
          : new Node(second.getId(), second.getLongitude(), second.getLatitude(), mergeTags(second, first));
    }

    return first.getMetadata().getTimestamp() < second.getMetadata().getTimestamp() ? new Node(first.getId(),
        first.getLongitude(), first.getLatitude(), mergeTags(first, second), first.getMetadata())
        : new Node(second.getId(), second.getLongitude(), second.getLatitude(), mergeTags(second, first),
            second.getMetadata());
  }
  ...
  private OsmWay mergeWays(OsmWay first, OsmWay second) {
    if (Objects.isNull(first.getMetadata()) || Objects.isNull(second.getMetadata())) {
      return new Random().nextBoolean() ? new Way(first.getId(), OsmModelUtil.nodesAsList(first),
          mergeTags(first, second))
          : new Way(second.getId(), OsmModelUtil.nodesAsList(second), mergeTags(second, first));
    }

    return first.getMetadata().getTimestamp() < second.getMetadata().getTimestamp() ? new Way(first.getId(),
        OsmModelUtil.nodesAsList(first), mergeTags(first, second), first.getMetadata())
  ...
  private <O extends OsmEntity> List<? extends OsmTag> mergeTags(O olderEntity, O newerEntity) {
    ...
    newerTags.forEach((key, value) -> olderTags.merge(key, value, (v1, v2) -> v1));

    return olderTags.entrySet().stream().map(entry -> new Tag(entry.getKey(), entry.getValue())).toList();
  }
