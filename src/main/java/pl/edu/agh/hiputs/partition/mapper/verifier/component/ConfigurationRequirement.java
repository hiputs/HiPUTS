...
public class ConfigurationRequirement implements Requirement{
  ...
  public boolean isSatisfying(Graph<JunctionData, WayData> graph) {
    return !modelConfigService.getDetector2Strategy().isEmpty() &&
        !modelConfigService.getCorrector2Strategy().isEmpty() &&
        ...
        modelConfigService.getModelConfig().getWayTypes().length > 0 &&
        new File(modelConfigService.getModelConfig().getSpeedLimitsFilePath()).exists() &&
