...
public class Osm2InternalModelMapperImpl implements Osm2InternalModelMapper{
  ...
  private Restriction osmToInternal(OsmRelation osmRelation,
      Map<String, List<Edge<JunctionData, WayData>>> osmWayId2Ends) {
    ...
    if (viaNodeId != null) {
      ...
      // to edge
      members.stream().filter(member -> member.getRole().equals("to")).findFirst().ifPresent(member -> {
        if (osmWayId2Ends.containsKey(String.valueOf(member.getId()))) {
          osmWayId2Ends.get(String.valueOf(member.getId()))
              .stream()
      ...
      // from edge
      members.stream().filter(member -> member.getRole().equals("from")).findFirst().ifPresent(member -> {
        if (osmWayId2Ends.containsKey(String.valueOf(member.getId()))) {
          osmWayId2Ends.get(String.valueOf(member.getId()))
              .stream()
  ...
  private boolean isComplete(Restriction restriction) {
    return StringUtils.isNotBlank(restriction.getId()) && StringUtils.isNotBlank(restriction.getViaNodeId())
        && StringUtils.isNotBlank(restriction.getFromEdgeId()) && StringUtils.isNotBlank(restriction.getToEdgeId())
        && Objects.nonNull(restriction.getType());
  }

  private Stream<Edge<JunctionData, WayData>> flatMapOsmWayToEdges(OsmWay osmWay,
      Map<String, List<Edge<JunctionData, WayData>>> osmWayId2Edges) {
