  ...
  public void detect(Graph<JunctionData, WayData> graph) {
    ...
    if (!typesIncompatibilities.isEmpty()) {
      context.setDetectionReport(String.format("%s - found wrong connections:\n%s", getClass().getSimpleName(),
          formatReportForIncompatibilities(typesIncompatibilities)));

      context.setPreparedCorrector(
          new WrongConnectionsCorrector(typesIncompatibilities, correctorStrategyFactory.getFromConfiguration()));
    }
