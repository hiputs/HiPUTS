...
public class OsmPartitionerVisualizer {
    ...
    private StringBuilder graphStylesBuilder = new StringBuilder("""
    ...
    protected pl.edu.agh.partition.model.graph.Graph<JunctionData, WayData> map;


    public OsmPartitionerVisualizer(pl.edu.agh.partition.model.graph.Graph<JunctionData, WayData> map) {
        this.map = map;
    ...
    protected void buildGraphStructure() {
        for (pl.edu.agh.partition.model.graph.Node<JunctionData, WayData> osmNode : map.getNodes().values()) {
            Node node = this.graph.addNode(osmNode.getId());
            node.setAttribute("label", osmNode.getId().substring(0, 3));
            node.setAttribute("xy", longitude2plain(osmNode.getData().getLon(), osmNode.getData().getLat()), latitude2plain(osmNode.getData().getLat()));
        }

        for (pl.edu.agh.partition.model.graph.Edge<JunctionData, WayData> osmEdge : map.getEdges().values()) {
    ...
    private String generateStyleFromUUID(String id) {
        return id + " { shape: line; fill-color: #" + Integer.toHexString(id.hashCode()%4096) + "; arrow-size: 0px, 0px; }\n";
    }
}
