...
import pl.edu.agh.partition.model.graph.Node;
...
public class TrivialPatchPartitioner implements PatchPartitioner {
  ...
  public Graph<PatchData, PatchConnectionData> partition(Graph<JunctionData, WayData> graph) {
    ...
    patchId2patch.entrySet().stream().map(es ->
        es.getValue().getData().getNodes().values().stream()
          .flatMap(nodes -> nodes.getOutgoingEdges().stream())
          .map(e -> {
            Edge<PatchData, PatchConnectionData> newPatchConnection = new Edge<>(es.getKey() + "->" + e.getData().getPatchId());
